...
# mdb_load -f ~/printer_data/config/data.mdb.bak -s -T ~/printer_data/database/

if command -v /usr/bin/mdb_dump &> /dev/null
then
    if $history_only
    then
        echo "mdb_dump found! Exporting history table from data.mdb to ~/printer_data/config/data.mdb.bak"
        mdb_dump -s history -n ~/printer_data/database/data.mdb -f ~/printer_data/config/data.mdb.bak
    else
        echo "mdb_dump found! Exporting ALL tables data.mdb to ~/printer_data/config/data.mdb.bak"
        mdb_dump -a -n ~/printer_data/database/data.mdb -f ~/printer_data/config/data.mdb.bak
    fi
else
